<template>
	<view class="allBg flexBox">
		<view class="rod-mume">
			<view v-for="i in mumeArr" :key="i.id" :class="['mume-item',{'mume-item-act':i.id==mumeAct}]" @click="mumeClick(i.id)"> 
				<span>{{i.name}}</span>
			</view>
		</view>
...
<script>	
	import { DSorderPage } from '@/api/page/index.js'
	export default {
		data() {
			return {
		...
		onShow() {
			let _that = this;
			uni.$once('ESiteOdDetail',function(data){
				_that.mumeClick(0)
			})
		},
		...
		methods: {
			...
			initData(){
				DSorderPage(this.listQuery).then((res) => {
					if(res.code == 200){
